...
module Work::DashboardHelper
  ...
  def today_work_summary(item_type_list = [] , user = nil)
    ...
    item_type_list.each do |item_type|
       ...
       item_summary_list.append("On #{item_type.classify.constantize.model_name.human.pluralize} 总共 #{item_list.count} 事件, 其中 " + events.collect{|e| "#{e[1]}次" +I18n.t("helpers.events.#{e[0]}")}.join(",")) if events.present?
    end
