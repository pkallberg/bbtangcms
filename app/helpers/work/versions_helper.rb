module Work::VersionsHelper
  ...
  def filter_drop_down_li(obj = "Version", col = '',count = 20)
    ...
    if obj.column_names.include? col.to_s
      ...
      list = obj.all.collect{|v| ["#{v.send(col.to_sym)}",v.send(col.to_sym)]}.uniq
      ...
      if col.eql? "whodunnit"
        list = obj.all.collect{|v| ["#{find_user(v.send(col.to_sym))}",v.send(col.to_sym)]}.uniq
      end
      if col.eql? "item_id"
        list = obj.find(:all, :order => "id desc", :limit => count).collect{|v| ["#{(v.send(col.gsub("_id",'').to_sym))}",v.send(col.to_sym)]}.uniq
      end
      if col.eql? "item_type"
        list = obj.all.collect{|v| ["#{v.send(col.to_sym).classify.constantize.model_name.human.pluralize}",v.send(col.to_sym)]}.uniq
      end
      content = list.collect{|l| raw "<li>#{link_to l[0],work_versions_path(version_params({col.to_sym =>l[1]})) }</li>"}.join
      foot = "</ul></li>"
