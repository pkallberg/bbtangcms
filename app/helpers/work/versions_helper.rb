module Work::VersionsHelper
  ...
  def version_filter_drop_down_li(obj = "Version", col = '',count = 20)
    ...
    if obj.column_names.include? col.to_s
      ...
      if col.eql? "item_id"
        ...
        list = obj.group(col.to_sym).collect{|item| ["#{item.send(col.gsub("_id",''))}",item.send(col)]}.uniq
      end
      if col.eql? "item_type"
        ...
        list = obj.group(col.to_sym).collect{|item| ["#{item.send(col).classify.constantize.model_name.human.pluralize}",item.send(col)]}.uniq
      end
